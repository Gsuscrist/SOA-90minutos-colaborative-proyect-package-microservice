...
export class GetAllPackagesRequestSaga {
    ...
    async listenForGetAllPackagesRequests(): Promise<void> {
        ...
        try {
            ...
            channel.consume(this.queueName, (msg: any) => {
                if (msg) {
                    signale.info('GetAllPackages request received:', msg.content.toString());
                    const content: any = JSON.parse(msg.content.toString());
                    // TODO: Procesar la solicitud para obtener todas las paqueterías
                    signale.info('Processed GetAllPackages request:', content);
                    // Ejemplo de enviar una respuesta de confirmación
                    const responseMessage = { message: 'Response to GetAllPackages request' };
                    const responseExchange = process.env.RABBIT_EXCHANGE_ADMINISTRATION || 'default';
                    const responseRoutingKey = process.env.RABBIT_ROUTING_KEY_GET_ALL_PACKAGES_REQ_ADMINISTRATION || 'default';
